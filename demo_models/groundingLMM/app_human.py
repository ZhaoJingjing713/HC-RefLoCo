...
def parse_args(args):
    ...
    parser.add_argument("--json_file", default="/home/v-jinjzhao/datasets/rec_human_celeb_ready/person_annts_val_ready.json")
    parser.add_argument("--objective", default='person')
    parser.add_argument("--image_path", default="/home/v-jinjzhao/datasets/rec_human_celeb_ready/images_v2/val_ready")
    parser.add_argument("--save_path", default="./output_human/groundingLMM/", type=str)
    parser.add_argument("--version", default="ckpt/GLaMM-FullScope")
...
def inference(input_str, all_inputs, follow_up, generate):
    ...
    prompt = f"Please provide the position of the person depicted as: \"{input_str}\""
    ...
    if "[SEG]" in text_output:
        ...
        # cv2.imwrite(os.path.join(args.save_path, 'test.jpg'), cv2.cvtColor(save_img, cv2.COLOR_RGB2BGR))
        bboxes_preds, max_mask = get_bbox_from_mask(pred_masks)
        # save_img2=prepare_mask(input_image, image_np, [max_mask[None]], text_output, color_history)
    else:
        bboxes_preds = [[0, 0, 0, 0]]
        save_img2=image_np
    ...
    return output_str, output_image, bboxes_preds, pred_masks
...
if __name__ == "__main__":
    ...
    save_preds_path = os.path.join(output_path, 'preds.jsonl')
    if(os.path.exists(save_preds_path)):
        pred_file=open(save_preds_path, 'r+')
        num_lines = pred_file.readlines()
    else:
        pred_file=open(save_preds_path, 'w')
        num_lines = []
    
    for annt in tqdm(json_data[len(num_lines):]):
        img_name = annt['file_name']
        img_path = os.path.join(img_folder, img_name)

        inputs = {'image': img_path, 'boxes': []}
        text_output, img_output, bboxes, masks = inference(annt['caption'], inputs, False, False)
        # draw the bbox
        # for bbox in bboxes:
        #     cv2.rectangle(img_output, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3]),), (0, 255, 0), 2)
        #     cv2.rectangle(max_mask_img, (int(bbox[0]), int(bbox[1])), (int(bbox[2]), int(bbox[3]),), (0, 255, 0), 2)
        # # plot the two images in one row
        # plt.figure(figsize=(10, 10), dpi=300)
        # plt.subplot(1, 2, 1)
        # plt.imshow(img_output)
        # plt.subplot(1, 2, 2)
        # plt.imshow(max_mask_img)
        # plt.show()
            
        # cv2.imwrite(os.path.join(output_path, annt['annt_id']+'.jpg'), cv2.cvtColor(img_output, cv2.COLOR_RGB2BGR))
        # cv2.imwrite(os.path.join(output_path, annt['annt_id']+'_max_mask.jpg'), cv2.cvtColor(max_mask_img, cv2.COLOR_RGB2BGR))
        pred_file.write(json.dumps({'annt_id': annt['annt_id'], 'text_output':text_output, 'pred_bboxes': bboxes})+'\n')
        pred_file.flush()
    # break
    pred_file.close()