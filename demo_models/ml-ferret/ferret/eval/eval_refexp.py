...
class RefExpEvaluatorFromJsonl(object):
    ...
    def summarize(self,
        ...
        for item_img, item_ann in tqdm(zip(self.refexp_gt['images'], self.refexp_gt['annotations'])):
            ...
            iou = box_iou(predict_boxes, target_bbox, mode='iou', is_aligned=True)
            mean_iou = box_iou(predict_boxes.mean(0).view(-1, 4), target_bbox, mode='iou', is_aligned=True)
            for k in self.k:
                if k == 'upper bound':
...
if __name__ == '__main__':
    ...
    parser.add_argument('--prediction_file', default='refexp_result/ref_human_7b',help='prediction_file')
    parser.add_argument('--annotation_file', default='/home/v-jinjzhao/dev/datasets/coco2014/finetune_refcoco_val.json', help='annotation_file')
