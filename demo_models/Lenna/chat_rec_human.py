...
def parse_args(args):
    ...
    parser.add_argument("--json-path", default="/home/v-jinjzhao/datasets/rec_human_celeb_ready/person_annts_val_ready.json")
    parser.add_argument("--images-path", default="/home/v-jinjzhao/datasets/rec_human_celeb_ready/images_v2/val_ready/")
...
def main(args):
    ...
    pred_file = open(os.path.join(args.output_path, 'prediction_during_eval.jsonl'), 'a+')
    pred_list=[i.readline() for i in pred_file]
    for annt in tqdm(all_annots[len(pred_list):]):
        ...
        prompt =f"Provide coordinates for the bounding box around the person mentioned in: \"{g_dino_caption}\""
        ...
        pred_file.write(json.dumps({"annt_id": annt['annt_id'],    
